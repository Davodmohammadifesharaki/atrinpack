...
export const categoriesOperations = {
  create: async (categoryData: { name: string; type: 'product' | 'news'; description?: string }) => {
    const { data, error } = await supabase
      .from('categories')
      .insert([categoryData])
      .select()
  ...
  update: async (id: string, categoryData: Partial<{ name: string; type: 'product' | 'news'; description: string }>) => {
    const { data, error } = await supabase
      .from('categories')
      .update(categoryData)
      .eq('id', id)
...
// Hook for categories
export const useCategories = (type?: 'product' | 'news') => {

      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50 p-4">
          <div className="relative max-w-4xl w-full">
            <img
              src={images[currentIndex] ? URL.createObjectURL(images[currentIndex]) : existingImages[currentIndex]}
            ...
            {allowDownload && (
              <button
                onClick={() => downloadImage(existingImages[currentIndex] || URL.createObjectURL(images[currentIndex]))}
            ...
            {(images.length + existingImages.length) > 1 && (
              <>
                ...
                <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black bg-opacity-50 text-white px-4 py-2 rounded-full font-bold">
                  {currentIndex + 1} / {images.length + existingImages.length}
                </div>
              </>
            )}
          </div>
